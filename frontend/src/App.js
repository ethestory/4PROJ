...
{/* Vue Collection */}
          {currentCollection ? (
            <div style={{padding: '20px', border: '2px solid #28a745', borderRadius: '10px'}}>
              ...
              {/* Liste des flux de la collection */}
              {collectionFeeds.length > 0 && (
                <div style={{marginBottom: '20px', padding: '15px', backgroundColor: '#f8f9fa', border: '1px solid #ddd'}}>
                  ...
                  {collectionFeeds.map(feed => (
                    <div key={feed.id} style={{border: '1px solid #ddd', padding: '10px', margin: '5px 0', backgroundColor: 'white'}}>
                      <h5 style={{marginTop: '0'}}>{feed.title}</h5>
                      <p style={{fontSize: '12px', color: '#666'}}>{feed.url}</p>
                      <p style={{fontSize: '12px'}}>Ajouté par: {feed.added_by} le {formatDate(feed.added_at)}</p>
                      {feed.last_updated && (
                        <p style={{fontSize: '12px', color: '#28a745'}}>Dernière sync: {formatDate(feed.last_updated)}</p>
                      )}
                    </div>
                  ))}
                </div>
